# GPT3: in UA123 change run_shell_command as run_ipython_code , interaction insert code in open ipython ， and get erro or result return as observation. => UA1234: how can open the ipython , and then dynamic interaction insert code by run_ipython_code?
...
def run_ipython_code(code: str) -> str:
    ...
    try:
        result = ipython_shell.run_cell(code, store_history=True)
        if result.error_in_exec:
            return f"Error: {result.error_in_exec}"
        elif result.result is not None:
            return str(result.result)
        else:
            return "Code executed successfully with no return value."
